...
// ── helpers ─────────────────────────────────────────────────────────────────
var kw = function (pattern, weight, isRegex) {
    if (isRegex === void 0) { isRegex = false; }
    return (__assign({ pattern: pattern, weight: weight }, (isRegex ? { isRegex: isRegex } : {})));
};
// ── 1 · Reasoning Depth ────────────────────────────────────────────────────
var reasoning = [
    // Chinese
...
var codeTech = [
    ...
    kw('session', 0.35),
    kw('middleware', 0.45),
    kw('authentication', 0.55),
    kw('authorization', 0.55),
    kw('permission', 0.45),
    kw('role-based', 0.50),
    kw('access control', 0.55),
    ...
    kw('消息队列', 0.55),
    kw('design', 0.30),
    kw('architecture', 0.50),
    kw('migration', 0.45),
    kw('rollback', 0.50),
    kw('load balancer', 0.55),
    kw('cache', 0.40),
    kw('message queue', 0.55),
    // regex: code fences
    kw('```', 0.60),
    kw('import ', 0.40),
    kw('const ', 0.35),
...
var taskSteps = [
    ...
    kw('处理', 0.25),
    kw('先.*再.*然后', 0.65, true),
    // Multi-requirement patterns (Chinese enumeration with 、)
    kw('包括.*、.*、', 0.50, true),
    kw('要求.*、.*、', 0.50, true),
    kw('需要.*和.*和', 0.45, true),
    kw('完整的', 0.35),
...
// ── Export ───────────────────────────────────────────────────────────────────
exports.KEYWORDS = (_a = {},
    _a[types_1.Dimension.REASONING] = reasoning,
    _a[types_1.Dimension.CODE_TECH] = codeTech,
    _a[types_1.Dimension.TASK_STEPS] = taskSteps,
    _a[types_1.Dimension.DOMAIN_EXPERT] = domainExpert,
    _a[types_1.Dimension.OUTPUT_COMPLEX] = outputComplex,
    _a[types_1.Dimension.CREATIVITY] = creativity,
    _a[types_1.Dimension.CONTEXT_DEPEND] = contextDepend,
    _a[types_1.Dimension.MESSAGE_LENGTH] = messageLength,
    _a);
